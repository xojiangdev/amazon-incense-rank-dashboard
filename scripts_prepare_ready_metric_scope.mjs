...
if (fullTargets.length !== 42 || targets.length !== 39 || expectedRows !== 314 || excludedTargets.length !== 3 || excludedKeywordCount !== 56) throw new Error(`Unexpected final scope: full=${fullTargets.length}, targets=${targets.length}, rows=${expectedRows}, excluded=${excludedTargets.length}/${excludedKeywordCount}`);
const scope = { status: "ready_partial_recovery_scope", createdAt: new Date().toISOString(), canonicalTargetFile: "rank_targets.json", expectedListings: targets.length, expectedRows, excluded, excludedKeywordCount, targets };
const output = path.join(root, "rank_targets_ready_excluding_3_temporary_listings.json");
